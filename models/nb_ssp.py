"""NB-SSP (Negative Binomial Stable-Beta Scaled Process) model."""
...
class NegBintSBSP():
    ...
    def draw(self, seed=None):
        ...
        self.parameters = self.beta, self.sigma, self.tilting, self.r
        if seed is None:
            np.random.seed(self.seed)
        else:
            np.random.seed(seed)
        num_new_daily_users = np.zeros(self.D, dtype=int)
        ...
        unit_level_total_retrigger = np.zeros(total_number_of_users[-1])
        ...
        for d in tqdm(range(1, self.D+1)):
            ...
            num_new_users_on_day_d = num_new_daily_users[d-1]
    ...
    def sample_retrigger_counts_new(self, d, num_samples, parameters):
        beta, sigma, tilting, r = parameters
    ...
    def get_proba_news(self, D_pilot, D_follow, parameters):
        '''
    ...
    def get_proba_news_freq(self, D_pilot, D_follow, parameters, frequency):
        '''
            Computes the parameter that drives the number of new users triggering
            in the experiment with a given frequency
        '''
        beta, sigma, tilting, r = parameters
    ...
    def mean_number_new_users(self, D_pilot, D_follow, N_at_D_pilot, parameters, frequency=None):
        '''
        Computes the mean number of new users to be observed in D_follow
    ...
    def ci_number_new_users(self, D_pilot, D_follow, N_at_D_pilot, parameters, width=0.95, frequency=None):
        '''
        Computes the 100*width% centered confidence intervals for the number
        of new users.
        '''
        beta, sigma, tilting, r = parameters
        if frequency is None:
    ...
    def total_retriggers_old_users(self, D_pilot, D_follow, counts_previous_retriggers, parameters, N_MC=1234):
        '''
    ...
    def total_retriggers_new_users(self, D_pilot, D_follow, N_at_D_pilot, parameters, N_MC=1234, threshold=10):
    ...
    def predict_counts_news(self, D_pilot, D_follow, N_at_D_pilot, counts, parameters):
    ...
    def make_approx_log_like(self, sfs, D):
    ...
    def make_approx_log_like_fixed_r(self, sfs, D):
    ...
    def fit_log_like(self, matrix, num_its=10,
    ...
    def fit_approx_log_like(self, sfs, D, num_its=10,
                            bnds=((.01, 100), (.1, .999), (.1, 100), (.1, 100),),
                            status=False):
    ...
    def fit_approx_log_like_fixed_r(self, sfs, D, num_its=10,
    ...
    def make_cost_function_regression(self, D0, N0, observed_counts, norm=2, frequency=None):
    ...
    def fit_regression(self, D0, N0, observed_counts, norm=2, frequency=None, num_its=10, status=False):

        bnds = ((.01, 100), (.01, .999), (.01, 100), (1, 100),)
        optimal_values_, optimal_params_ = np.zeros(num_its), np.zeros([num_its, 4])
