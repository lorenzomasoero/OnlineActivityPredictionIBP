"""TG-SSP (Tilted Gibbs Stable-Beta Scaled Process) model."""
...
class GD():
    ...
    def predict_counts_news(self, N, M, params, counts):
        K = counts[-1]
        return np.concatenate([counts, K+self.mean(N, M, K, params)])
    ...
    def make_EFPF(self, sfs, N):
        ...
        Output :
            cost_function <function>; cost function from using norm on true_cts
        ...
        def EFPF(params):
            ...
            cost = K*np.log(sigma) - (K+tilting+1) * np.log(
                beta+sigma*spb(
                    1-sigma,
                    np.arange(1, N+1)).sum()) + gln(K+tilting+1) - gln(tilting+1) + \
                        (tilting+1)*np.log(beta) + \
                            np.inner(sfs, bln(np.arange(1, len(sfs)+1)-sigma, N - np.arange(1, len(sfs)+1) + 1))
            return - cost
    ...
    def make_cost_function(self, train_counts, from_, up_to, norm):
        ...
        Input :
            ...
            from_ < int 0<= from < N > index of lowest sample
            up_to < int ; from_ < up_to <= N> index of highest sample
    ...
    def regression(self, train_counts, num_its, norm, status):
        ...
        '''
        # beta sigma tilting
        bnds = ((0, 10000), (.0001, .99999), (0, 10000),)
        optimal_values_, optimal_params_ = np.zeros(num_its), np.zeros([num_its, 3])
        N = len(train_counts)
        from_, up_to = 0, N
    ...
    '''
       FIT WITH GEOMETRIC LIKELIHOOD
    '''

    def fit_geometric_marginal(self, first_trigger_counts, D,
                               num_its=10, bnds=((0, 10000), (.01, .999), (0, 10000),), status=False):

        optimal_values_, optimal_params_ = np.zeros([num_its]), np.zeros([num_its, 3])

        cost = self.make_geometric_marginal(first_trigger_counts, D)
        if status == True:
            for it in tqdm(range(num_its)):
                devol = optimize.differential_evolution(cost, bnds)
                optimal_params_[it] = devol.x
    ...
    def make_geometric_marginal(self, new_users_per_day, d):
        new_users_per_day = np.asarray(new_users_per_day)
        N = np.sum(new_users_per_day)

        def EFPF(params):
            beta, alpha, c = params
            gammad = alpha * spb(1-alpha, np.arange(1, d+1)).sum()
            cost = (N * np.log(alpha) + (c+1) * np.log(beta) -
                    (N + c + 1) * np.log(beta + gammad) +
                    gln(N + c + 1) - gln(c + 1) +
                        np.inner(new_users_per_day, bln(1 - alpha, np.arange(1, d+1)))
                    )
            return -cost
