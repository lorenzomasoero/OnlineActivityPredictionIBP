"""
Unseen species estimator via linear programming (Zou et al.).

Requires the optional dependency ``cvxopt``. Install with::
...
try:
    ...
    raise ImportError(
        "The unseen species estimator requires cvxopt. "
        "Install it with: pip install cvxopt"
    )
...
def predict_counts_unseen(N, M, sfs, kappa):
    '''
    ...
    Output :
        total : <array> predicted accumulation curve
    '''
...
def new_variants_with_frequency_unseen(bins, densities, N, M, r):
    '''
        Predict number of variants with frequency r or up to r.

        Input :
            bins <array> values in [0,1] of variants' frequencies
            densities <array> corresponding "height" of the histogram h(x) for each x in bins
        Output :
            prediction : <array> predicted new variants with given frequency
    '''
