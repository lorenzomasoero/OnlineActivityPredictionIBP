...
Output:
    ../results/uci_all_results.npy

Usage:
    cd SubmissionAOAS/fitting
    python fit_uci_all_models.py
"""
...
# Add bundled utils to path (Lorenzo's implementation of record)
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "utils"))
# Reproducibility: seed from env SEED (default 0) for the stochastic restarts
...
try:
    from utils_beta_geom import predict_beta_geom
    HAS_BETA_GEOM = True
except ImportError:
    HAS_BETA_GEOM = False
    print("Warning: utils_beta_geom not available (cmdstanpy/arviz dependency). Skipping BG model.")
...
# Configuration
# ============================================================
DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "uci")
...
for exp in tqdm(experiments, desc="Fitting experiments"):
    ...
    # ----------------------------------------------------------
    # SFS (site frequency spectrum): how many users were active on exactly 1, 2, ..., D0 days
    pilot_binary = (matrix_pilot > 0).astype(int)
    days_active_per_user = pilot_binary.sum(axis=0)  # shape: (N_customers,)
    
    # Only consider users active in pilot
    active_in_pilot = days_active_per_user > 0
    days_active_pilot_users = days_active_per_user[active_in_pilot]
    
    sfs = np.bincount(days_active_pilot_users)[1:]  # sfs[j-1] = # users active on exactly j days
    
    # Retrigger counts for pilot users (for NB-SSP)
    retrigger_counts_pilot = matrix_pilot[:, active_in_pilot].sum(axis=0)  # total triggers per user in pilot
    
    # First trigger counts (cumulative new users per day in pilot)
    ...
    try:
        ...
        # Predict new users: mean(N=D0, M=D1, K=N_pilot, params)
        gd_pred_news = gd.mean(D0, D1, N_pilot, gd_params)
        gd_U_hat = gd_pred_news[-1]  # predicted new users at end of follow-up
        
        # CI: credible_interval(N, M, K, params, width) -> (lo_array, hi_array)
    ...
    try:
        ...
        # Compute CI manually (scipy.stats.poisson.interval API changed)
