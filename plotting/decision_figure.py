...
def load_and_predict(dataset, k=None):
    ...
    if dataset == 'rees46':
        ...
        all_results = np.load(os.path.join(RESULTS_DIR, 'rees46', f'rees46_rolling_k{k}_results.npy'), allow_pickle=True).item()
    else:
        all_results = np.load(os.path.join(RESULTS_DIR, dataset, f'{dataset}_all_results.npy'), allow_pickle=True).item()
        experiments = []
